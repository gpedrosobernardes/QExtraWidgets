...
class QEmojiSortFilterProxyModel(QSortFilterProxyModel):
    ...
    def filterAcceptsRow(self, source_row: int, source_parent: QModelIndex) -> bool:
        ...
        aliases = idx.data(QEmojiItem.QEmojiDataRole.AliasRole)
        pattern = self.filterRegularExpression().pattern()

        if pattern and not pattern.lower() in aliases:
            return False

        category = idx.data(QEmojiItem.QEmojiDataRole.CategoryRole)
        if self._category_filter is not None and self._category_filter != category:
            return False

        recent = idx.data(QEmojiItem.QEmojiDataRole.RecentRole)
        if self._recent_filter is not None and self._recent_filter != recent:
            return False

        favorite = idx.data(QEmojiItem.QEmojiDataRole.FavoriteRole)
